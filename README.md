# Spectral Classification Toolkit
...
# Motivation 
Laser-Induced Breakdown Spectroscopy (LIBS) produces high-dimensional spectra whose interpretation can benefit from machine learning methods. This repository illustrates a complete, general-purpose workflow — from exploratory data analysis to supervised classification — for experimental LIBS spectra.

Beyond achieving high classification accuracy, the project investigates whether models trained on one specimen can generalize to different samples belonging to the same source. This validation strategy, which better reflects real-world applications, is inspired by previous studies in the LIBS literature [1,2].


# Project description:  

This repository contains a machine learning toolkit and workflow for the classification of Laser-Induced Breakdown Spectroscopy (LIBS) spectra: exploratory data analysis, preprocessing utilities, and supervised classification models.

As a worked example, the workflow is applied to a bone classification case study: the objective is to classify bone spectra according to the individual from which the sample originated and to evaluate the model's ability to generalize to spectra acquired from different bones belonging to the same individual.
...
└── README.md
```
- **notebooks/**: exploratory data analysis (01) and the main machine learning workflow (02).
...
# Workflow
1. Exploratory data analysis (`01_EDA.ipynb`): merge and condense raw spectra, inspect and visualize them
2. Preprocessing
   - normalization
...
6. Generalization test using unseen bone specimens (case study)
